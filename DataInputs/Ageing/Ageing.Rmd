...
labels <- data.frame(AGE = c(rep(12, 3)), 
                             #rep(11.5,3), 
                             #rep(11,3)), 
                     name=c(#"gam_k5", 
                            "vonB", 
...
require(ggrepel)
png(paste0(plotsGo, "BBn/fit.png"),width=8, height=8, units = "in", res=420)
all <- ggplot() + 
...
png(paste0(plotsGo, "BBn/fit_inset.png"),width=10, height=8, units = "in", res=420)
...
png(paste0(plotsGo, "BBn/resid.png"),width=10, height=8, units = "in", res=420)
ggplot() + 
...
write.csv(x = df, paste0(plotsGo, "BBn/summary_stats.csv"))
...
preds_var_1y2$end_age <- c(seq(3, 12, 0.1), rep(NA,10))
preds_var_1y2$start_age <- seq(2, 12, 0.1)
...
age <- ggplot() + 
  ...
  geom_label(data=labels, aes(AGE, diff, label=long), hjust=c(0.5, 0.25, 0.5))+
...
sh <- ggplot() + 
  ...
  geom_label(data=labels, aes(value, diff, label=long), hjust=c(0.5, 0.25, 0.5))+
...
require(patchwork)
png(paste0(plotsGo, "BBn/growth.png"),width=10, height=8, units = "in", res=420)
...
# Fixed effects:  linf + k + tzero ~ 1 
#           Value Std.Error  DF  t-value p-value
# linf  162.14355  4.425928 381 36.63493  0.0000
# k       0.18965  0.010959 381 17.30484  0.0000
# tzero   0.19711  0.062629 381  3.14725  0.0018
#lvb.plt(plt)
#lvb.plt1(plt)
#lvb.plt1
#equation: 162.14355 * (1 - exp(-0.18965 * (x - 0.19711)))
...
# mod_re_3 <- gam(data=tpd_long, method="REML", HEIGHT ~ s(AGE, k=3) + s(ID, bs="re") + s(ID, Scall.no, bs="re"))
# Error: Model has more coefficients than data
#mod_re_3 <- gamm4(HEIGHT ~ s(AGE), data=tpd_long, random = ~ (1 + AGE| shell.ID))
# predict and plot this one # 2023-04-14
# compare to mod_re_3.2
...
# labels <- unique(preds_long[!preds_long$name %in% c("gam_re_3", "gam_re_3.2", "gam_re_5", "gam_re_5.2"), c("AGE", "name", "value")])
labels <- data.frame(AGE = c(rep(11, 2), 
                             rep(11,3), 
                             rep(11,3)), 
                     name=c("gam_k5", "vonB", "gam_k3", "gam_re_3f", "gam_re_5f", "gam_re_3.2f", "gam_re_5.2f", "gam_re_3.3f"))
labels <- left_join(labels, unique(dplyr::select(preds_long, AGE, name, value)))

require(ggrepel)
ggplot() + 
  geom_line(data=preds_long[!preds_long$name %in% c("gam_re_3", "gam_re_3.2", "gam_re_5", "gam_re_5.2"),], aes(AGE, value, colour=name), show.legend=F) + 
  geom_text_repel(data=labels,
                  aes(AGE, value, colour=name, label=name),
                  show.legend=F,
                  max.overlaps=Inf,
                  hjust="left",
                  direction="y",
...
# Compare differences in first 6 years
preds_var <- preds_long %>%
  filter(name %in% c("gam_k3", "gam_k5", "gam_re_3f", "gam_re_3.2f",'gam_re_3.3f', "gam_re_5f", "gam_re_5.2f", "vonB"))%>%
  group_by(AGE, name) %>%
  summarize(value = mean(value),
            se = mean(se),
            up = mean(vonB_up),
...
png(paste0(plotsGo, "Sab/resid.png"),width=10, height=8, units = "in", res=420)
ggplot() + 
  geom_point(data=resids_long[resids_long$name %in% c("vonB", "gam_re_3", "gam_re_5"),], aes(AGE, value)) + 
  geom_smooth(data=resids_long[resids_long$name %in% c("vonB", "gam_re_3", "gam_re_5"),], aes(AGE, value), method="lm", colour="black") + 
  facet_wrap(~name2) +
...
# 
# ggplot() + 
#   geom_point(data=resids_long, aes(HEIGHT, HEIGHT-value, colour=name)) + 
#   geom_smooth(data=resids_long, aes(HEIGHT, HEIGHT-value, colour=name), method="lm", se=F) + 
#   geom_abline(data=resids_long, aes(intercept=0, slope=1)) #+ 
#   #facet_wrap(~name)
...
```{r]}
df <- NULL
for(mod in c("mod3", "mod_re_3", "mod_re_3.2", "mod_re_3.3", "mod5", "mod_re_5", "mod_re_5.2")){
  print(nrow(df))
  if(!any(class(get(mod))=="list")) {
    row <- data.frame(name=deparse(formula(get(mod))), 
                      AIC=AIC(get(mod)), 
...
preds_var_1y2 <- full_join(preds_var_1y[11:nrow(preds_var_1y),], data.frame(AGE=rep(NA, 10))) - preds_var_1y[1:nrow(preds_var_1y),]
preds_var_1y2$end_age <- c(seq(3, 11, 0.1), rep(NA,10))
preds_var_1y2$start_age <- seq(2, 11, 0.1)
preds_var_1y2 <- pivot_longer(preds_var_1y2, cols = names(preds_var_1y2)[!names(preds_var_1y2) %in% c("AGE", "end_age", "start_age")])
preds_var_1y2 <- select(preds_var_1y2, -AGE)
names(preds_var_1y2)[names(preds_var_1y2) == "value"] <- "diff"
preds_var_1y2$AGE <- preds_var_1y2$end_age
...
age <- ggplot() + geom_line(data=preds_var_1y[preds_var_1y$end_age <7 & !is.na(preds_var_1y$diff),], aes(end_age, diff, group=name)) +
  ylab("Growth in previous year (mm)") +
  xlab("Age") +
  theme_bw()

sh <- ggplot() + geom_line(data=preds_var_1y[preds_var_1y$end_age <7 & !is.na(preds_var_1y$diff),], aes(value, diff, group=name)) +
  ylab("Growth in previous year (mm)") +
  xlab("Shell height (mm)") + 
  theme_bw()

require(patchwork)
age + sh + plot_annotation(title="Sable")

# SH of 95 is around 15mm (80-95mm)



# final models only
age <- ggplot() + 
  geom_line(data=preds_var_1y[preds_var_1y$end_age <7 & 
                                !is.na(preds_var_1y$diff) & preds_var_1y$name %in% 
                                c("gam_re_3f", "gam_re_5f", "vonB"),], 
            aes(end_age, diff, colour=name)) +
  ylab("Growth in previous year (mm)") +
  xlab("Age") + 
  scale_colour_discrete(name="Model", labels=c("GAM, 3 knots", "GAM, 5 knots", "vonB")) +
  theme_bw()

sh <- ggplot() + geom_line(data=preds_var_1y[preds_var_1y$end_age <7 & !is.na(preds_var_1y$diff) & preds_var_1y$name %in%c("gam_re_3f", "gam_re_5f", "vonB"),], aes(value, diff, colour=name)) +
  ylab(NULL) +
  xlab("Shell height (mm)") + 
  scale_colour_discrete(name="Model", labels=c("GAM, 3 knots", "GAM, 5 knots", "vonB")) +
  theme_bw()
...
require(patchwork)
age + sh + 
  plot_annotation(title="Browns Bank north") +
