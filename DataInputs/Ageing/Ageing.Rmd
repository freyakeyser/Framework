...
require(ggplot2)
require(lubridate)
load("C:/Users/keyserf/Documents/temp_data/testing_results_framework3.Rdata")
```
...
require(gamm4)
#mod_re_3.3 <- gamm4(HEIGHT ~ s(AGE), data=tpd_long, random = ~ (1 + AGE| shell.ID))
...
preds_vonB <- preds_se[grep(x=preds_se$name, pattern = "vonB"),]
...
if(bank=="Sab") {labels <- data.frame(AGE = c(rep(10, 3)), 
                             #rep(11.5,3), 
                             #rep(11,3)), 
                     name=c(#"gam_k5", 
                            "vonB", 
...
all <- ggplot() + 
  ...
  geom_line(data=preds_long[!preds_long$name %in% c("gam_k3", "gam_re_3", "gam_re_3.2", "gam_k5", "gam_re_5", "gam_re_5.2"),], 
            aes(AGE, value, group=name), show.legend=F) + 
  geom_ribbon(data=preds_long[!preds_long$name %in% c("gam_k3", "gam_re_3", "gam_re_3.2", "gam_k5", "gam_re_5", "gam_re_5.2"),], 
...
inset <- ggplot() + 
  ...
  theme_bw() +
  ylab(NULL) +
  xlab(NULL) + facet_wrap(~name, ncol=1) +
  theme(strip.text = element_blank()) 

png(paste0(plotsGo, "/", bank, "/fit_inset.png"),width=10, height=8, units = "in", res=420)
all + inset_element(p = inset, align_to = "full", left=0.5, bottom=0.1, right = 0.975, top=0.5)
dev.off()

ggplot() + geom_line(data=preds_long[preds_long$name %in% c("gam_k3", "gam_re_3", "gam_re_3.2", "gam_re_3.3f"),], aes(AGE, value, group=shell.ID, colour=name)) + facet_wrap(~name) +
    geom_line(data=vonB, aes(AGE, value), linewidth=1)

ggplot() + geom_line(data=preds_long[preds_long$name %in% c("gam_k5", "gam_re_5", "gam_re_5.2"),], aes(AGE, value, group=shell.ID, colour=name)) + facet_wrap(~name) +
  geom_line(data=vonB, aes(AGE, value), linewidth=1)
...
preds_var <- preds_long %>%
  filter(name %in% c("gam_k3", "gam_k5", "gam_re_3f", "gam_re_3.2f","gam_re_5f", "gam_re_5.2f", "vonB", "gam_re_3.3f")) %>%
  group_by(AGE, name) %>%
...
resids_long$name2 <- factor(resids_long$name2, labels=c("gam_k3", "gam_k5", "GAMM, 3 knots", "GAMM, 5 knots", "vonB"))

png(paste0(plotsGo, "/", bank, "/resid.png"),width=10, height=8, units = "in", res=420)
ggplot() + 
  geom_point(data=resids_long[resids_long$name %in% c("vonB", "gam_re_3", "gam_re_5"),], aes(AGE, value)) + 
  geom_smooth(data=resids_long[resids_long$name %in% c("vonB", "gam_re_3", "gam_re_5"),], aes(AGE, value), method="lm", colour="black") + 
  facet_wrap(~name2) +
...
resids_long[resids_long$name %in% c("vonB", "gam_re_3", "gam_re_5"),] %>%
  group_by(name, AGE) %>%
...
ggplot() +
  geom_point(data=resids_long[resids_long$name %in% c("vonB", "gam_re_3", "gam_re_5"),], aes(HEIGHT, HEIGHT-value, colour=name)) +
  geom_smooth(data=resids_long[resids_long$name %in% c("vonB", "gam_re_3", "gam_re_5"),], aes(HEIGHT, HEIGHT-value, colour=name), method="lm", se=F) +
  geom_abline(data=resids_long[resids_long$name %in% c("vonB", "gam_re_3", "gam_re_5"),], aes(intercept=0, slope=1)) #+
  #facet_wrap(~name)
...
```{r}
df <- NULL
l.bar <- survey.obj$BBn$model.dat$l.bar[nrow(survey.obj$BBn$model.dat)]
for(mod in c("vonB", "mod_re_3", 
  ...
  if(!mod == "vonB"){
    if(mod=="mod_re_3") pred <- "gam_re_3f"
    if(mod=="mod_re_5") pred <- "gam_re_5f"
    
    preds_sub <- preds_simple[preds_simple$name==pred,]
    
    if(!any(class(get(mod))=="list")) {
      row <- data.frame(name=deparse(formula(get(mod))), 
                        AIC=AIC(get(mod)), 
                        r.sq = summary(get(mod))$r.sq,
                        scale.est=summary(get(mod))$scale,
                        dev=summary(get(mod))$dev,
                        H10=gammit::predict_gamm(get(mod), newdata = data.frame(AGE=10, ID=1, Scall.no=1), exclude=c("ID", "Scall.no"), re_form=NA)$prediction,
                        H5=gammit::predict_gamm(get(mod), newdata = data.frame(AGE=5, ID=1, Scall.no=1), exclude=c("ID", "Scall.no"), re_form=NA)$prediction,
                        H2=gammit::predict_gamm(get(mod), newdata = data.frame(AGE=2, ID=1, Scall.no=1), exclude=c("ID", "Scall.no"), re_form=NA)$prediction,
                        AFR=preds_sub$AGE[which.min(abs(preds_sub$value-l.bar))],
                        AR=preds_sub$AGE[which.min(abs(90-preds_sub$value))],
                        APR=preds_sub$AGE[which.min(abs(75-preds_sub$value))])
      
      row$K_1yprior_to_R <- preds_sub$value[which.min(abs(preds_sub$AGE-(row$AR)))] - preds_sub$value[which.min(abs(preds_sub$AGE-(row$AR-1)))]
      row$K_1yprior_to_avg_FR <- preds_sub$value[which.min(abs(preds_sub$AGE-(row$AFR)))] - preds_sub$value[which.min(abs(preds_sub$AGE-(row$AFR-1)))]
      row$K_2_to_5 <- row$H5 - row$H2
      
      
    }
    if(any(class(get(mod))=="list")) {
      row <- data.frame(name=paste0(deparse(formula(get(mod)$gam)), ", ",  deparse(formula(get(mod)$mer))), 
                        AIC=AIC(get(mod)$mer), 
...
### testing out ranges
```{r}
preds_var_1y <- pivot_wider(select(preds_var, -se, -up, -low), id_cols="AGE")
...
preds_var_1y2 <- select(preds_var_1y2, -AGE)
...
labels <- data.frame(AGE = c(5.7, 5.5, 6), 
                             #rep(11.5,3), 
                             #rep(11,3)),
                     name=c(#"gam_k5", 
                            "vonB", 
...
age <- ggplot() + 
  geom_line(data=preds_var_1y[preds_var_1y$end_age <7 & !is.na(preds_var_1y$diff) & preds_var_1y$name %in% c("gam_re_3f","gam_re_5f","vonB"),], aes(end_age, diff, group=name)) +
  geom_label(data=labels, aes(AGE, diff, label=long), hjust=c(0.9, 0, 1))+
...
sh <- ggplot() + 
  geom_line(data=preds_var_1y[preds_var_1y$end_age <7 & !is.na(preds_var_1y$diff) & preds_var_1y$name %in% c("gam_re_3f","gam_re_5f","vonB"),], aes(value, diff, group=name)) +
  geom_label(data=labels, aes(value, diff, label=long), hjust=c(0.9, 0, 1))+
...
require(patchwork)
png(paste0(plotsGo, bank, "/growth.png"),width=10, height=8, units = "in", res=420)
...
png(paste0(plotsGo, "/", bank, "/fit_vb.png"),width=8, height=8, units = "in", res=420)
ggplot() + 
  geom_point(data=tpd_long, aes(AGE, HEIGHT), size=1)+
  ...
  ylab("Shell height (mm)") +
  xlab("Age") +
  annotate(geom="text", x=7.5, y=80, label=paste0("l_inf = ", round(vonBpars$linf, 1)), hjust=0) +
  annotate(geom="text", x=7.5, y=70, label=paste0("k = ", round(vonBpars$k, 1)), hjust=0) + 
  annotate(geom="text", x=7.5, y=60, label=paste0("t_0 = ", round(vonBpars$tzero, 1)), hjust=0) 
dev.off()


png(paste0(plotsGo, "/", bank, "/resid_vb.png"),width=8, height=8, units = "in", res=420)
ggplot() + 
...
require(patchwork)
png(paste0(plotsGo, bank, "/growth_vb.png"),width=10, height=8, units = "in", res=420)
age + sh + plot_layout(guides="collect")  
dev.off()



tster <- tpd_long %>% dplyr::filter(AGE <= 10)
tster$shell.ID <- paste0(tster$ID,"_",tster$Scall.no)
mod.gam.re3.1 <- gamm4(HEIGHT ~ s(AGE,bs='cs',k=3), data=tster, random = ~ (1 | shell.ID))
preds_exp$mn.gr3.1 <- predict(object = mod.gam.re3.1$gam, newdata = preds_exp)
preds_exp$se.gr3.1 <- predict(object = mod.gam.re3.1$gam, newdata = preds_exp, se.fit=T)$se.fit
preds_exp$UCI.gr3.1 <- preds_exp$mn.gr3.1 + 2*preds_exp$se.gr3.1
preds_exp$LCI.gr3.1 <- preds_exp$mn.gr3.1 - 2*preds_exp$se.gr3.1

mod.gam.re3 <- gam(data=tpd_long, method="REML", HEIGHT ~ s(AGE, k=3) + s(ID, bs="re") + s(Scall.no, bs="re"))
preds_exp$mn.gr3 <- gammit::predict_gamm(model = mod.gam.re3, newdata = preds_exp, re_form=NA)$prediction
preds_exp$se.gr3 <- gammit::predict_gamm(model = mod.gam.re3, newdata = preds_exp, se=T, re_form=NA)$se
preds_exp$UCI.gr3 <- preds_exp$mn.gr3 + 2*preds_exp$se.gr3
preds_exp$LCI.gr3 <- preds_exp$mn.gr3 - 2*preds_exp$se.gr3

mod.gam.re3.2 <- gamm4(data=tster, HEIGHT ~ s(AGE, bs="cs", k=3), random = ~(1|Scall.no+ID))
preds_exp$mn.gr3.2 <- gammit::predict_gamm(model = mod.gam.re3.2$gam, newdata = preds_exp, re_form=NA)$prediction
preds_exp$se.gr3.2 <- gammit::predict_gamm(model = mod.gam.re3.2$gam, newdata = preds_exp, se=T, re_form=NA)$se
preds_exp$UCI.gr3.2 <- preds_exp$mn.gr3.2 + 2*preds_exp$se.gr3.2
preds_exp$LCI.gr3.2 <- preds_exp$mn.gr3.2 - 2*preds_exp$se.gr3.2


ggplot() + 
  geom_point(data=tster, aes(AGE, HEIGHT, colour=shell.ID,group=shell.ID)) +
  geom_line(data=preds_exp, aes(AGE, mn.gr3, group=shell.ID), colour="blue") +
  theme(legend.position = 'none') +
  geom_ribbon(data=preds_exp, aes(ymin=LCI.gr3,ymax=UCI.gr3,x=AGE), fill=NA, colour="blue", linetype="dashed") +

  geom_line(data=preds_exp, aes(AGE, mn.gr3.1, group=shell.ID)) +
  theme(legend.position = 'none') +
  geom_ribbon(data=preds_exp, aes(ymin=LCI.gr3.1,ymax=UCI.gr3.1,x=AGE), fill=NA, colour="black", linetype="dashed") +
  
  geom_line(data=preds_exp, aes(AGE, mn.gr3.2, group=shell.ID), colour="red") +
  theme(legend.position = 'none') +
  geom_ribbon(data=preds_exp, aes(ymin=LCI.gr3.2,ymax=UCI.gr3.2,x=AGE), fill=NA, colour="red", linetype="dashed")
