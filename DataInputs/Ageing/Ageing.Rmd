...
#resids_long <- pivot_longer(resids, cols=names(resids)[!names(resids) %in% c("AGE", "HEIGHT")])
...
labels <- data.frame(AGE = c(rep(12, 2), 
                             rep(11.5,3), 
                             rep(11,2)), 
                     name=c("gam_k5", "vonB", "gam_k3", "gam_re_3f", "gam_re_5f", "gam_re_3.2f", "gam_re_5.2f"))
...
ggplot() + geom_line(data=preds_long[preds_long$name %in% c("gam_k3", "gam_re_3", "gam_re_3.2"),], aes(AGE, value, group=shell.ID, colour=name)) + facet_wrap(~name) +
...
preds_var <- preds_long %>%
  filter(name %in% c("gam_k3", "gam_k5", "gam_re_3f", "gam_re_3.2f","gam_re_5f", "gam_re_5.2f", "vonB")) %>%
  group_by(AGE, name) %>%
...
# ggplot() + geom_point(data=resids_long, aes(AGE, value, colour=name)) + facet_wrap(~name) +
#   ggtitle("residuals (real - predicted)")
# 
# resids_long %>%
#   group_by(name, AGE) %>%
...
# preds_var_1y <- data.frame(name = row.names(preds_var_1y), preds_var_1y)
# names(preds_var_1y) <- paste0("AGE_", preds_var_1y[1,])
# names(preds_var_1y)[1] <- "name"
# row.names(preds_var_1y) <- NULL
# preds_var_1y <- preds_var_1y[-1,]
# 
# start <- 2:ncol(preds_var_1y)
# end <- 10:ncol(preds_var_1y)
# 
# preds_var_1y[,end] - preds_var_1y[,start]
...
ggplot() + geom_line(data=preds_var_1y[preds_var_1y$end_age <7 & !is.na(preds_var_1y$diff),], aes(end_age, diff, colour=name)) +
  ylab("1-year growth (mm)")

ggplot() + geom_line(data=preds_var_1y[preds_var_1y$end_age <7 & !is.na(preds_var_1y$diff),], aes(value, diff, colour=name)) +
...
preds <- data.frame(AGE = seq(min(tpd_long$AGE), max(tpd_long$AGE), 0.25))
preds$vonB <- predict(cheat, newdata=preds)
# from https://derekogle.com/fishR/2019-12-31-ggplot-vonB-fitPlot-1
boots <- car::Boot(cheat)
vonB_se <- confint(boots)
...
# Compare differences in first 6 years
preds_var <- preds_long %>%
  filter(AGE %in% seq(1,12,0.5) & 
           name %in% c("gam_k3", "gam_k5", "gam_re_3f", "gam_re_3.2f","gam_re_5f", "gam_re_5.2f", "vonB"))%>%
  group_by(AGE, name) %>%
  summarize(value = mean(value),
            se = mean(se),
            up = mean(vonB_up),
...
ggplot() + geom_point(data=preds_var, aes(AGE, std_dev))
ggplot() + geom_point(data=preds_var, aes(AGE, diff))

print(preds_var[preds_var$AGE %in% 1:7,])
