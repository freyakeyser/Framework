...
mod_re_3.4 <- gamm4(data=tster, HEIGHT ~ s(AGE, bs="cs", k=3), random = ~(1|Scall.no/ID))
...
mod_re_5.4 <- gamm4(data=tster, HEIGHT ~ s(AGE, bs="cs", k=5), random = ~(1|Scall.no/ID))
...
all <- ggplot() + 
  ...
  geom_line(data=preds_long[preds_long$name %in% c("gam_re_3.4f", "gam_re_5.4f", "vonB"),], 
            aes(AGE, value, group=name), show.legend=F) + 
  geom_ribbon(data=preds_long[preds_long$name %in% c("gam_re_3.4f", "gam_re_5.4f", "vonB"),], 
...
preds_var <- preds_long %>%
  filter(name %in% c("gam_k3", "gam_k5", "gam_re_3f", "gam_re_3.2f","gam_re_3.4f", "gam_re_5f", "gam_re_5.2f", "gam_re_5.4f", "vonB", "gam_re_3.3f")) %>%
  group_by(AGE, name) %>%
...
resids_long$name2 <- factor(resids_long$name2, labels=c("gam_k3", "gam_k5", "gam_re_3", "GAMM, 3 knots", "gam_re_5", "GAMM, 5 knots", "vonB"))

png(paste0(plotsGo, "/", bank, "/resid.png"),width=10, height=8, units = "in", res=420)
ggplot() + 
  geom_point(data=resids_long[resids_long$name %in% c("vonB", "gam_re_3.4f", "gam_re_5.4f"),], aes(AGE, value)) + 
  geom_smooth(data=resids_long[resids_long$name %in% c("vonB", "gam_re_3.4f", "gam_re_5.4f"),], aes(AGE, value), method="lm", colour="black") + 
  facet_wrap(~name2) +
...
labels <- data.frame(AGE = c(5.2, 5.5, 6), 
                             #rep(11.5,3), 
                             #rep(11,3)),
                     name=c(#"gam_k5", 
                            "vonB", 
...
mod.gam.re3 <- gam(data=tpd_long, method="REML", HEIGHT ~ s(AGE, bs='cs', k=3) + s(ID, bs="re") + s(Scall.no, bs="re"))
