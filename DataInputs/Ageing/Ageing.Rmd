...
png(paste0(plotsGo, "/", bank, "/fit_vb", fr, ".png"),width=6.5*1.5, height=5*1.5, units = "in", res=420)
ggplot() + 
  geom_point(data=tpd_long, aes(AGE, HEIGHT), size=1)+
  ...
  annotate(geom="text", x=7.5, y=80, label=Linf, parse=T, hjust=0) +
  annotate(geom="text", x=7.5, y=70, label=K, parse=T, hjust=0) + 
  annotate(geom="text", x=7.5, y=60, label=t_0, parse=T, hjust=0) 
dev.off()


png(paste0(plotsGo, "/", bank, "/resid_vb", fr, ".png"),width=6.5*1.5, height=5*1.5, units = "in", res=420)
ggplot() + 
...
png(paste0(plotsGo, bank, "/growth_vb", fr, ".png"), width=6.5*1.5, height=4*1.5, units = "in", res=420)
age + sh + plot_layout(guides="collect")  
dev.off()
