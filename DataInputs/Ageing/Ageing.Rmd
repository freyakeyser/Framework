...
if(bank=="BBn") {# Fresh ages from TPD
  ...
  locs <- dplyr::left_join(tpd_long, all.surv.dat[all.surv.dat$bank=="BBn" & all.surv.dat$state=="live",]) %>%
    dplyr::select("year", "bank", "tow", "lon", "lat") %>%
    dplyr::distinct()
...
if(bank=="Sab") {
  ...
  # get locations
  #tpd_long
  locs <- unique(all.surv.dat[all.surv.dat$bank==tpd_long$bank & 
                 all.surv.dat$year==tpd_long$year &
                 all.surv.dat$tow %in% tpd_long$tow, c("year", "bank", "tow", "lon", "lat")])
  locs <- sf::st_as_sf(locs, coords=c("lon", "lat"), crs=4326)
