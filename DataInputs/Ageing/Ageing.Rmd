...
```{r}
bank <- "BBn"
boundaries <- github_spatial_import("survey_boundaries", "survey_boundaries.zip", quiet=T, direct_fns="C:/Users/keyserf/Documents/Github/Assessment_fns/")

boundaries$ID <- gsub(x=boundaries$ID, pattern=".shp", replacement="", fixed=T)
...
if(bank=="BBn") {# Fresh ages from TPD
  ...
  # get locations
  #tpd_long
  locs <- dplyr::left_join(tpd_long, all.surv.dat[all.surv.dat$bank=="BBn" & all.surv.dat$state=="live",]) %>%
    dplyr::select("year", "bank", "tow", "lon", "lat","depth") %>%
    dplyr::distinct()
  locs <- sf::st_as_sf(locs, coords=c("lon", "lat"), crs=4326)
  
  shells <- tpd_long %>% 
    dplyr::group_by(cruise, year, tow, bank) %>%
    dplyr::summarize(n_samples = length(unique(Scall.no)))
  
  locs <- left_join(locs, shells)
  
  summary(locs$depth)
  summary(all.surv.dat[all.surv.dat$bank=="BBn",]$depth)
  
  png(paste0(plotsGo, "/", bank, "/age_map.png"),width=8, height=8, units = "in", res=420)
  bbn_map <- ggplot() + 
    geom_sf(data=boundaries[boundaries$ID=="BBn",])+
    geom_sf(data=locs, aes(shape=as.factor(year), size=n_samples)) +
...
if(bank=="Sab") {
  ...
  # get locations
  #tpd_long
  locs <- unique(all.surv.dat[all.surv.dat$bank==unique(tpd_long$bank) & 
                 all.surv.dat$year %in% unique(tpd_long$year) &
                 all.surv.dat$tow %in% tpd_long$tow, c("year", "bank", "tow", "lon", "lat", "depth")])
  locs <- sf::st_as_sf(locs, coords=c("lon", "lat"), crs=4326)
  summary(locs$depth)
  shells <- tpd_long %>% 
    dplyr::group_by(cruise, year, tow, bank) %>%
  ...
  locs <- dplyr::left_join(locs, shells)
  
  summary(locs$depth)
  summary(all.surv.dat[all.surv.dat$bank==unique(tpd_long$bank),]$depth)

  png(paste0(plotsGo, "/", bank, "/age_map.png"),width=8, height=8, units = "in", res=420)
  ggplot() + geom_sf(data=boundaries[boundaries$ID=="Sab",])+
    geom_sf(data=locs, aes(size=n_samples)) + 
    scale_size_continuous(name="Number of shells") +
    theme_bw() +
    theme(legend.position = c(1,0.1), legend.justification = "right", legend.background=element_rect(fill=NA))
  dev.off()
}
```
...
### Running models
```{r}
#lvb.nlme(AGE.dat = tpd_long, random.effect="shell.ID") # does not converge
...
yheightnext <- 90
xagenext <- log(-((yheightnext/vonBpars$linf) -1))/(-vonBpars$k) + vonBpars$tzero
xagenext2 <- xagenext + 1
yheightnext2 <- vonBpars$linf * (1 - exp(-vonBpars$k * (xagenext2 - vonBpars$tzero)))
yheightnext2 - yheightnext


png(paste0(plotsGo, "/", bank, "/fit_vb", fr, "_2.png"),width=6.5, height=5, units = "in", res=420)
ggplot() + 
  geom_point(data=tpd_long, aes(AGE, HEIGHT), size=1)+
  ...
  geom_segment(aes(y=yheightnext2, yend=yheightnext2, x=min(preds_long$AGE)*0.75, xend=xagenext2), colour="forestgreen", linetype="dashed") +
  geom_segment(aes(y=min(preds_long$value)*0.75, yend=yheightnext2, x=xagenext2, xend=xagenext2), colour="forestgreen", linetype="dashed") +
  annotate(geom="text", x=min(preds_long$AGE)*0.75, y=yheightnext2, label=round(yheightnext2,2), colour="forestgreen", hjust=-0.5, vjust=-0.1)+
  annotate(geom="text", x=xagenext2, y=min(preds_long$value)*0.75, label=round(xagenext2,2), colour="forestgreen", vjust=-1, hjust=-0.1)+
  ggtitle(bank)+
  #ylim(50,170) +
  theme_bw() +
  theme(panel.grid=element_blank())+
