...
bnk <- "BBn"
cf2 <- condFac(na.omit(mw.dat.all[[bnk]]),bank.dat[[bnk]],model.type='gam_f',dirct=direct_fns, plt=T)
...
mw.dat.all$BBn
mw.dat.mod <- mw.dat.all$BBn[complete.cases(mw.dat.all$BBn),]
...
ggplot() +geom_point(data=cf.data$BBn$CF.data, aes(x=year, y=CF)) + 
  geom_line(data=pred.dat, aes(x=year, y=fit2, group=1), colour="red") +
  geom_line(data=pred.dat[pred.dat$sh==100,], aes(x=year, y=fit, group=1), colour="blue")

ggplot() +geom_point(data=mw.dat.mod, aes(x=sh,y=wmw)) + 
  geom_line(data=pred.dat, aes(x=sh, y=fit, group=year), colour="red") +
  geom_line(data=pred.dat, aes(x=sh, y=fit2, group=year), colour="blue") +
  facet_wrap(~year)
