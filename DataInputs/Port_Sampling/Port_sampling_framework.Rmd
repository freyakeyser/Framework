...
shfps <- shfps %>%
  dplyr::mutate(bin = cut(sh, breaks=seq(5,210,5), right=F)) %>% # gotta go above 200 since these are estimates (not 5mm bins)
  dplyr::group_by(surv.year, bin) %>%
  dplyr::summarize(value=n())
...
bins <- data.frame(shf=seq(5,214.9,0.1),
                   name=rep(seq(5,210,5), each=50))
yearsbins <- data.frame(years=rep(unique(shfps$surv.year), each=length(seq(5,210,5))), 
                        name=rep(seq(5,210,5), length(unique(shfps$surv.year))))
bins <- full_join(bins, yearsbins, relationship="many-to-many")
