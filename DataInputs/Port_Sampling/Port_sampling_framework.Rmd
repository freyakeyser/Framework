...
ggplot() +
  ...
  geom_segment(data=medians2[medians2$bank %in% c("BBn", "Sab"),],mapping=aes(x=monthyr, xend=xend,y=annual_med, yend=annual_med), colour="blue", size=1) +
...
ps_tab <- port.sampling %>% 
  filter(!bank %in% c("GBa", "GBb", "SPB")) %>%
  group_by(sfa,bank) %>%
  summarize(total=length(unique(ID)),
            years=length(unique(year)),
            recent=max(year),
            months = length(unique(month)),
            ltm_mc = round(median(mc),2),
            sd_mc = round(sd(mc),2),
            min_mc = round(min(mc),2), 
            max_mc = round(max(mc),2))
ps_tab$main<-NA
...
#                                ) +
png(paste0(plotsGo, bank, "/shf_ps", fr, ".png"), width=6.5, height=8, units = "in", res=420)
ggplot() + 
