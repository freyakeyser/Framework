...
png(paste0(plotsGo, "medians_long", fr, ".png"), width=6.5, height=9, units = "in", res=420)
ggplot() +
  geom_line(data=medians[medians$bank %in% c("BBn", "Sab"),], aes(monthyr, monthly_med)) +
  geom_errorbar(data=medians[medians$bank %in% c("BBn", "Sab"),], aes(monthyr, ymin=monthly_med-(1.96*monthly_sd), ymax=monthly_med+(1.96*monthly_sd)))+
  facet_wrap(~bank, ncol=1) +
  ylab(ylabel) +
  xlab("Month")+
  theme_bw() +
...
ps_tab <- port.sampling %>% 
  ...
  summarize(total=length(unique(ID)),
            ...
            min_mc = min(mc), 
            max_mc = max(mc))
ps_tab$months[ps_tab$bank == "BBn"] <- "1-12"
ps_tab$months[ps_tab$bank=="Sab"] <- "3-12"
