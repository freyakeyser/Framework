...
if(french==F) ylabel <- "Meat weight (median, g)"
...
ggplot() +
  ...
  geom_segment(data=medians2[medians2$bank %in% c("BBn", "Sab"),],mapping=aes(x=monthyr, xend=xend,y=annual_med, yend=annual_med), colour="blue", linewidth=1) +
  geom_errorbar(data=medians[medians$bank %in% c("BBn", "Sab"),], aes(monthyr, ymin=monthly_med-(1.96*monthly_sd), ymax=monthly_med+(1.96*monthly_sd)), width=0)+
  geom_point(data=medians[medians$bank %in% c("BBn", "Sab"),], aes(monthyr, monthly_med)) +
  geom_text(data=medians2[medians2$bank %in% c("BBn", "Sab"),], aes(monthyr + months(6), 0, label=nsamples_yr), vjust=0.5, hjust=1.1, colour="blue")+
...
for(i in 1:length(years))
{
  ...
  bbn.dat$sh[bbn.dat$year == years[i]] <- ((bbn.dat$meat_weight[bbn.dat$year == years[i]]/exp(fix.int))^ (1/fix.slope))*100
...
ggplot() + 
  geom_density_ridges(data=bbn.dat, aes(x = sh, y = as.factor(year), height = stat(density)),
...
bins <- data.frame(shf=rep(seq(65,209.9,0.1), length(unique(shfps$year))), name=rep(rep(seq(70,210,5), each=50), length(unique(shfps$year))), year=rep(unique(shfps$year), each=1450))
shfps$name <- as.numeric(shfps$name)

shfps <- full_join(shfps, bins)

shfps <- left_join(data.frame(year = min(shfps$year, na.rm=T):max(shfps$year, na.rm=T)), shfps)
shfps$bank <- "BBn"

shfps <- dplyr::select(shfps, bank, year, name, shf, density)
