...
for (rdata in c(#"framework1", "framework2", "framework3", "framework4", 
  "framework5"#, "framework6"
  )){
  ...
  for(b in c("BBn", "BBs", "GBb")){
    ...
    filleryears <- expand.grid(year=min(get(rdata)$survey.obj[[b]]$bankpertow$year):max(get(rdata)$survey.obj[[b]]$bankpertow$year), bank=b, liner="lined")

      bankpertow <- get(rdata)$survey.obj[[b]]$bankpertow %>%
        full_join(filleryears)%>%
        arrange(year)%>%
        dplyr::mutate(bank=b) %>%
...
gp <- ggplot(g.proper[!is.na(g.proper$g.proper),]) + geom_point(aes(x=year,y=g.proper),color="#FFD500",shape =21,size=3,fill="#FFD500") + 
                   ...
                   theme_few(base_size = 16) + ylab("Growth rate") + xlab("Year")
png(paste(plotsGo,"/growth_new.png",sep=""),width=10,height=8,res=920,units="in")
print(gp)
dev.off()
```
...
tacwrite <- tac2[, c("Year", "sfa", "bank", "TAC")] %>% arrange(Year, sfa, bank)
write.csv(file = paste0(plotsGo, "/fishery/tac.csv"), x=tacwrite)
