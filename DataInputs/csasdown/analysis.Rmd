...
framework1 <- list(SS.summary=SS.summary, merged.survey.obj=merged.survey.obj, survey.obj=survey.obj, cf.data=cf.data, surv.Clap.Rand, nickname=nickname)
load("C:/Users/keyserf/Documents/temp_data/testing_results_framework_80-95RSCS_oldMWSH_GBb.RData")
framework3 <- list(SS.summary=SS.summary, merged.survey.obj=merged.survey.obj, survey.obj=survey.obj, cf.data=cf.data, surv.Clap.Rand, nickname=nickname)
load("C:/Users/keyserf/Documents/temp_data/testing_results_framework_origRSCS_newMWSH_GBb.RData")
framework4 <- list(SS.summary=SS.summary, merged.survey.obj=merged.survey.obj, survey.obj=survey.obj, cf.data=cf.data, surv.Clap.Rand, nickname=nickname)
load("C:/Users/keyserf/Documents/temp_data/testing_results_framework_80-95RSCS_newMWSH_GBb.RData") 
framework6 <- list(SS.summary=SS.summary, merged.survey.obj=merged.survey.obj, survey.obj=survey.obj, cf.data=cf.data, surv.Clap.Rand, nickname=nickname)
load("C:/Users/keyserf/Documents/temp_data/testing_results_framework_75-90RSCS_oldMWSH_GBb.RData") 
framework2 <- list(SS.summary=SS.summary, merged.survey.obj=merged.survey.obj, survey.obj=survey.obj, cf.data=cf.data, surv.Clap.Rand, nickname=nickname)
load("C:/Users/keyserf/Documents/temp_data/testing_results_framework_75-90RSCS_newMWSH_GBb.RData") 
framework5 <- list(SS.summary=SS.summary, merged.survey.obj=merged.survey.obj, survey.obj=survey.obj, cf.data=cf.data, surv.Clap.Rand, nickname=nickname)
...
for(sfa in sfa){
  ...
  N <- ggplot() + geom_line(data=bankpertow[bankpertow$sfa==sfa & bankpertow$index=="N",],
                            aes(year, index.val, colour=bank, linetype=liner)) +
    geom_point(data=bankpertow[bankpertow$sfa==sfa & bankpertow$index=="N",],
               aes(year, index.val, colour=bank, shape=bank)) +
...
for(sfa in sfa){
  ...
  N <- ggplot() + geom_line(data=bankpertow_all[bankpertow_all$sfa==sfa & bankpertow_all$index=="N" & bankpertow_all$size=="R"& !is.na(bankpertow_all$recsize),],
                            aes(year, index.val, colour=bank, group=recsize)) +
    geom_point(data=bankpertow_all[bankpertow_all$sfa==sfa & bankpertow_all$index=="N" & bankpertow_all$size=="R"& !is.na(bankpertow_all$recsize),],
    ...
    geom_ribbon(data=bankpertow_all[bankpertow_all$sfa==sfa & bankpertow_all$index=="N" & bankpertow_all$size=="R"& !is.na(bankpertow_all$recsize),],
  ...
  I <- ggplot() + geom_line(data=bankpertow_all[bankpertow_all$sfa==sfa & bankpertow_all$index=="I" & bankpertow_all$size=="R"& !is.na(bankpertow_all$recsize),],
                            aes(year, index.val, colour=bank, group=recsize)) +
    geom_point(data=bankpertow_all[bankpertow_all$sfa==sfa & bankpertow_all$index=="I" & bankpertow_all$size=="R"& !is.na(bankpertow_all$recsize),],
               aes(year, index.val, colour=bank, shape=recsize)) +
    #geom_errorbar(data=bankpertow_all, aes(year, ymax=NPR+(NPR*NPR.cv), ymin=NPR-(NPR*NPR.cv), colour=bank), width=0) +
    geom_ribbon(data=bankpertow_all[bankpertow_all$sfa==sfa & bankpertow_all$index=="I" & bankpertow_all$size=="R"& !is.na(bankpertow_all$recsize),],
                aes(year, ymax=index.val+cv.val, ymin=index.val-cv.val, fill=bank, group=recsize), alpha=0.3)+
    theme_bw() +
    # facet_grid(factor(index, levels=c("N", "I"))~factor(size, levels=c("PR", "R", "FR")),
    #            scales="free_y")
  ...
  require(patchwork)
  if(french==F) png(filename=paste0(plotsGo, "/Survey/", nickname, "/timeseries_", sfa, "_compare_", MWSH, ".png"),width=6.5*1.25, height=6.5*1.25, units = "in", res=420)
  if(french==T) png(filename=paste0(plotsGo, "/Survey/", nickname, "/timeseries_", sfa, "_compare_fr_", MWSH, ".png"),width=6.5*1.25, height=6.5*1.25, units = "in", res=420)
  print(N+I + plot_layout(guides="collect") & theme(legend.position="bottom"))
  dev.off()
}
...
if(french==F){
  version <- "Model"
  cfts_old$version <- "original"
  cfts$version<- "proposed"
}

if(french==T){
  version <- "Modèle"
  cfts_old$version <- "original"
  cfts$version<- "proposé"
}
...
png(filename=paste0(plotsGo, "/Survey/", nickname, "/cfts_27.png"),width=6.5, height=9, units = "in", res=420)
...
# comparing to CF using new glmer MWSH model
...
for(sfa in sfa){
  ...
  clap <- ggplot() + geom_line(data=clappertow[clappertow$sfa==sfa,],
                            aes(years, index.val, colour=bank)) +
...
for(sfa in sfa){
  ...
  for(bank in banks){
    ...
    print(ggplot() + 
      ...
      geom_vline(data=shf, aes(xintercept=RS)) +
      geom_vline(data=shf, aes(xintercept=CS)) +
      ggtitle(bank)
    )
    dev.off()
    ...
    print(ggplot() + 
      ...
      scale_y_discrete(expand=c(0,0.9), limits=rev) +
      geom_vline(data=shf[shf$name>75 | is.na(shf$name),], aes(xintercept=RS)) +
      geom_vline(data=shf[shf$name>75 | is.na(shf$name),], aes(xintercept=CS)) +
      ggtitle(bank)
    )
    dev.off()
  }
...
# Growth
- No modelled and realized, just realized

-	Use 2023 csas update model growth compare to framework5 (new MWSH/75-90/new VB)

-	Add one panel for recruit growth too

```{r growth, echo=F, include=F}
load("Y:/Offshore/Assessment/Data/Model/2023/BBn/Model_input.RData")
growth <- data.frame(bank="BBn", year=mod.dat$BBn$year, g2=mod.dat$BBn$g2, gR2 = mod.dat$BBn$gR2, version="original")

load("Y:/Offshore/Assessment/Data/Model/2023/BBn/Model_input_framework_vonB_75-90RSCS_newMWSH.RData")
growth <- rbind(growth, 
                data.frame(bank="BBn", year=mod.dat$BBn$year, g2=mod.dat$BBn$g2, gR2 = mod.dat$BBn$gR2, version="proposed")
                )
load("Y:/Offshore/Assessment/Data/Model/2023/BBn/Model_input_framework_vonB_origRSCS_oldMWSH.RData")
growth <- rbind(growth, 
                data.frame(bank="BBn", year=mod.dat$BBn$year, g2=mod.dat$BBn$g2, gR2 = mod.dat$BBn$gR2, version="vonB only")
                )
growth <- pivot_longer(data = growth, cols = c("g2", "gR2"))

growth$name <- gsub(x=growth$name, pattern="g2", replacement="Fully recruited")
growth$name <- gsub(x=growth$name, pattern="gR2", replacement="Recruit")

growth$version <- factor(growth$version, levels=c("proposed", "vonB only", "original"))

if(french==T){
  growth$version <- gsub(x=growth$version, pattern="original", replacement="original")
  growth$version <- gsub(x=growth$version, pattern="proposed", replacement="proposé")
...
ggplot() + 
  ...
  theme_bw() + 
  scale_colour_manual(values=c("black", "darkorange", "blue"), name=NULL) +
...
for(sfa in 25:27){
  ...
  kg <- ggplot() + geom_point(data=fishery[fishery$sfa==sfa,], aes(year, catch/1000, colour=bank, shape=bank), size=2) +
  ...
  hm <- ggplot() + geom_point(data=fishery[fishery$sfa==sfa,], aes(year, effort, colour=bank, shape=bank), size=2) +
  ...
  cp <- ggplot() + geom_point(data=cpue[cpue$sfa==sfa,], aes(year, cpue, colour=bank, shape=bank), size=2) +
...
##### SOMETHING IS WRONG WITH FISH.DAT HERE. BBn NO LONGER MATCHES, AND REL F's ARE TOO HIGH
# Probably has to do with survey year bits above, but I ran out of time. 
...
for(b in banks) {
  ...
  if(!b=="Sab/Mid"){
    ...
    if(!b=="Ger") compare <- full_join(survey.obj[[b]]$model.dat[, c("year", "I")], survey.yr[survey.yr$bank==b,c("year", "bank", "mt")])
  }
...
relative.F$sfa[relative.F$bank %in% c("Ban", "Ban/Sab/Mid", "Sab/Mid")] <- as.numeric(25)
