...
framework2 <- list(SS.summary=SS.summary, merged.survey.obj=merged.survey.obj, survey.obj=survey.obj, cf.data=cf.data, surv.Clap.Rand)
load("C:/Users/keyserf/Documents/temp_data/testing_results_framework_80-95.RData") # 80-95
framework4 <- list(SS.summary=SS.summary, merged.survey.obj=merged.survey.obj, survey.obj=survey.obj, cf.data=cf.data, surv.Clap.Rand)
load("C:/Users/keyserf/Documents/temp_data/testing_results_framework_75-90.RData") #75-90
framework3 <- list(SS.summary=SS.summary, merged.survey.obj=merged.survey.obj, survey.obj=survey.obj, cf.data=cf.data, surv.Clap.Rand)
load("C:/Users/keyserf/Documents/temp_data/testing_results_framework_75-90_newareas_issue120.RData") # 80-95
framework5 <- list(SS.summary=SS.summary, merged.survey.obj=merged.survey.obj, survey.obj=survey.obj, cf.data=cf.data, surv.Clap.Rand)
...
for (rdata in c("framework2", "framework3", "framework4", "framework5")){
...
for(sfa in sfa){
  ...
  require(patchwork)
  png(filename=paste0(plotsGo, "/timeseries_", sfa, "_", nickname, ".png"),width=6.5*1.25, height=6.5*1.25, units = "in", res=420)
...
print(ggplot() + geom_point(data=cfts[cfts$sfa==26,], aes(year, CF, colour=bank)) +
        geom_line(data=cfts[cfts$sfa==26,], aes(year, CF, colour=bank, linetype=type, shape=bank)) +
        xlab(en2fr("Year", translate = french, custom_terms=rosetta_terms)) + 
        ylab(cflab) +
        theme_bw() +
...
print(ggplot() + geom_point(data=cfts[cfts$sfa==25,], aes(year, CF, colour=bank, shape=bank)) +
...
for(sfa in sfa){
  ...
  require(patchwork)
  png(filename=paste0(plotsGo, "/clap_prop_", sfa, "_", nickname, ".png"),width=6.5, height=9, units = "in", res=420)
...
growth <- data.frame(bank="BBn", year=mod.dat$BBn$year, g=mod.dat$BBn$g, g2=mod.dat$BBn$g2, version="original")

load("Y:/Offshore/Assessment/Data/Model/2023/BBn/Model_input_framework_vonB.RData")
growth <- rbind(growth, 
                data.frame(bank="BBn", year=mod.dat$BBn$year, g=mod.dat$BBn$g, g2=mod.dat$BBn$g2, version="new")
                )
growth <- pivot_longer(data = growth, cols = c("g", "g2"))

growth$name <- gsub(x=growth$name, pattern="g2", replacement="realized")
growth$name <- gsub(x=growth$name, pattern="g", replacement="modelled")

if(french==T){
  growth$version <- gsub(x=growth$version, pattern="old", replacement="vieux")
  growth$version <- gsub(x=growth$version, pattern="new", replacement="nouveau")
  growth$name <- gsub(x=growth$name, pattern="realized", replacement="actuel")
  growth$name <- gsub(x=growth$name, pattern="modelled", replacement="prédit")
}

png(filename=paste0(plotsGo, "/growth_BBn", fr, ".png"),width=6.5, height=4, units = "in", res=420)
ggplot() + 
  geom_line(data=growth, aes(year, value, colour=version, linetype=name)) +
...
for(b in c("BBn", "BBs", "Sab")){
  ...
  png(filename=paste0(plotsGo, "/surveymap_", b, fr, ".png"),width=6.5*2, height=4*2, units = "in", res=420)
  print(
    p + 
...
fishery <- fish_sf %>% 
  dplyr::filter(!bank %in% c("GBa", "GBb")) %>%
...
tac2 <- tac2[tac2$sfa %in% 25:26,]
tac2$year <- tac2$Year 
tac3$bank <- factor(tac3$Bank, levels=c("Sab/Mid/Ban", "Ban", "Sab/Mid", "BBn/BBs", "BBn", "BBs", "Ger"))
...
fishery2$bank <- factor(fishery2$bank, levels=c("Sab/Mid/Ban", "Ban", "Sab/Mid", "BBn/BBs", "BBn", "BBs", "Ger"))
tac2$bank <- factor(tac2$bank, levels=c("Sab/Mid/Ban", "Ban", "Sab/Mid", "BBn/BBs", "BBn", "BBs", "Ger"))
...
png(filename=paste0(plotsGo, "/tac_", nickname, ".png"),width=6.5, height= 8, units = "in", res=420)
print(tac25 + tac26 + plot_annotation(tag_levels=c("A", "B")) + plot_layout(design = areas))
dev.off()
...
fish.dat$surv.year[fish.dat$month %in% 6:12] <- fish.dat$year[fish.dat$month %in% 6:12]+1
...
for(b in banks) {
  if(b=="Sab/Mid"){
    ...
    sabmid <- full_join(sab, mid) %>%
      group_by(year) %>%
      summarize(I=sum(I, na.rm=T)/0.35) # survey catchability (between 0.2 and 0.5) 
    fishing <- survey.yr %>%
  ...
  compare$relative.F <- compare$mt / (compare$I/0.35) # survey catchability?
...
png(filename=paste0(plotsGo, "/relF_", nickname, ".png"),width=6.5, height= 8, units = "in", res=420)
print(exp25 + exp26 + plot_annotation(tag_levels=c("A", "B")) + plot_layout(design = areas))
dev.off()
...
png(filename=paste0(plotsGo, "/exploit_BBn_", nickname, ".png"),width=6.5, height= 4, units = "in", res=420)
