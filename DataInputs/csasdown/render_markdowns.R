...
markdowns <- lapply(
  ...
  function(x, filename){
    ...
    csasdown::render(filename, params = list(bank = x, banknum=which(x==banks)), run_pandoc = FALSE, verbose=T)
...
csasdown::render(to, run_pandoc = TRUE)
