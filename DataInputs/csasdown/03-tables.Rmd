...
ps_tab <- select(ps_tab, -X, -main)
names(ps_tab) <- c("SFA", "Management unit", "Number of samples", "Number of years with PS data", "Most recent year with PS data", "Number of months with PS data", "Long term median (mc)", "Standard deviation (mc)", "Minimum (mc)", "Maximum (mc)")
...
inc <- pivot_wider(select(inc, -X), id_cols=AGE, names_from = bank, values_from=c(n_inc, height))
inc <- inc[,c("AGE", "n_inc_Sab", "height_Sab", "n_inc_BBn", "height_BBn")]
names(inc) <- c("Age assigned", "Number of increments (Sab)", "Median height (Sab, SD)",  "Number of increments (BBn)", "Median height (BBn, SD)")

age_summary <- full_join(sum_sab, sum_bbn)
age_summary <- select(age_summary, bank, year, tows, shells, increments, -X, -ager)
...
csas_table(ps_tab, caption ="Port sampling data summary.") %>% 
  ...
  kableExtra::row_spec(0,bold=ifelse(ps_tab$`Management unit`%in% c("BBn", "Sab"), TRUE, FALSE))
