...
prod.dat$RPS3_offset <- c(rep(NA,3),prod.dat$RPS3[-((length(prod.dat$RPS3)-2):length(prod.dat$RPS3))])
...
# Kobe plot
#windows(11,11)
kobe.plt <- ggplot(ann.exploit,aes(x=B,y=exploit)) + geom_text(aes(label=year)) +  xlim(c(0,8200)) + 
                                    annotate(geom="rect",xmin=0,xmax=4000,ymin=0.03,ymax=Inf, fill = "red", alpha=0.5)+
                                    annotate(geom="rect",xmin=4000,xmax=Inf,ymin=0,ymax=0.03, fill="green", alpha = 0.5) +
                                    annotate(geom="rect",xmin=0,xmax=4000,ymin=0,ymax=0.03, fill="gold1", alpha = 0.6) +
                                    annotate(geom="rect",xmin=4000,xmax=Inf,ymin=0.03,ymax=Inf, fill="gold1", alpha = 0.6) +
...
# The phase plot
#windows(11,11)
phase.plt <- ggplot(ann.exploit,aes(x=B,y=exploit)) + geom_text(aes(label=year)) +  xlim(c(0,8200)) + 
...
r.strat <- data.frame(type = "bp",strat = 'dist', bp = 3.5e3,mn.mx = signif(min(prod.dat$RPS3,na.rm=T),digits=2), sd.mx = 0.2,rec.age=3,
...
# r.strat <- data.frame(type = "bp",strat = 'dist', bp = 7150,mn.mx = signif(min(prod.dat$RPS3,na.rm=T),digits=2), sd.mx = 0.2,rec.age=5,
...
m.strat <- data.frame(type = "bp",strat = 'dist', bp = 3e3,mn.mx = 2*signif(mean(prod.dat$m,na.rm=T),digits=2), sd.mx = 0.1,ar1=0,ar2=signif(p.m.pacf$data$Freq[2],digits=2)) #
...
Res <- proj.mod(n_sim=500,n_y = 200,model=mod,plot_url = plot_url, run = 'no_model_error', res_url = res_url,save.results = T, exp.scenario=seq(0,0.15,0.01) , 
                mod.run = seam.select,
                #base.yrs = 2008:2022,
...
Exp.res <- readRDS("D:/Framework/SFA_25_26_2024/RefPts/Results/Sab/R_75_FR_90/SEAM_1994_2022_vary_m_m0_0.2_qR_0.45_10_knots_g_original/RPs/SEAM_g_sv_0_1_dist_1_0.05_r_bp_dist_3500__0.035_0.23_m_bp_dist_3000__0.4_0.1.Rds")
...
```{r hcr-sims,include =F}
 ...
 # RUNME
hcr.strat <-data.frame(TRP = 5.4e3,TRP.exp = 0.2, # Try 80% of B@0
                       USR =   3e3, USR.exp = 0.05, # 40% of B@0
                       LRP = 1.35e3, LRP.exp = 0,exp.sd = 0.1) # Around 5500 is the Biomass at 0 catch in the 0.4, 4 knot scenario.
#g.strat <- data.frame(type = "cor",strat = 'dist', bp = 0,mn.mx = 1, sd.mx = 0.05,ar1=0,ar2=0) # The g correlation method seems to get g a bit too  high.
g.strat <- data.frame(type = "bp",strat = 'dist', bp = 8e3,mn.mx = 1, sd.mx = 0.05)
#g.strat <- data.frame(type = "lm",strat = "Linear", bp = "Model",mn.mx = 1, sd.mx = 0.05)
r.strat <- data.frame(type = "bp",strat = 'dist', bp = 3.5e3,mn.mx = signif(min(prod.dat$RPS3,na.rm=T),digits=2), sd.mx = 0.2,rec.age=3,
                     ar1=signif(p.rps.pacf$data$Freq[1],digits=2),ar2=0,rps.m.cor = 0.43,rps.m.lag = 2) # Note lag is -5, high RPS comes 5 years before high m due to RPS offset.
# r.strat <- data.frame(type = "bp",strat = 'dist', bp = 7150,mn.mx = signif(min(prod.dat$RPS3,na.rm=T),digits=2), sd.mx = 0.2,rec.age=5,
...
# An HCR model
hcr <- proj.mod(n_sim=500,n_y = 100,model=mod,plot_url = plot_url, run = 'no_model_error', res_url = res_url,save.results = T, mod.run = seam.select,
