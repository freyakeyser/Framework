...
relative.F$I[relative.F$year == 2022 & relative.F$bank == "Sab"] <- survey.obj$Sab$model.dat$I[survey.obj$Sab$model.dat$year == 2022]
...
p.sab.mod.ind.bm <-ggplot(mod.index.bm) + geom_text(aes(x=index.bm,y=mod.bm,label = substr(year,3,4))) +
                                          ...
                                          geom_abline(slope=1/mod.out$BUGSoutput$median$q,intercept=0)
...
p.bbn.mod.ind.bm <-ggplot(mod.index.bm) + geom_text(aes(x=index.bm,y=mod.bm,label = substr(year,3,4))) +
                                          scale_x_continuous(name="Survey Biomass Index (tonnes)") +
                                          scale_y_continuous(name="Model Biomass (tonnes)") +
                                          geom_abline(slope=1/med.q,intercept=0)
...
names(model.rp.tab.dat) <- c("$B_{MSY}_{30}$", "$B_{MSY}_{40}$","$B_0_{20}$",
                             "$B_{MSY}_{80}$","$B_0_{40}$","$RR_{target}$")

```

```{r survey-index-rp-table,include=T,echo=F}
csas_table(index.rp.tab.dat,caption="A summary of the reference points for each area calculated using the survey index based methods.  The LRPs include, 20\\% of the maximum survey biomass index ($LRP_{20}$), 30\\% of the median survey biomass index ($LRP_{30}$), and 40\\% of the median survey biomass index ($LRP_{40}$). The USRs include, 40\\% of the maximum survey biomass index ($USR_{40}$), 80\\% of the median survey biomass index ($USR_{80}$). The limit removal reference was calculated using the GEM method, while the target removal reference was the median removals during the five-year period of time in which the survey biomass index had the least variability.")
```

```{r model-rp-table,include=T,echo=F}
csas_table(model.rp.tab.dat,caption="A summary of the reference points for SFA 25A and SFA 26A calculated using the MSY simulations.  The LRPs include, 20\\% of the median biomass using simulations with no fishing ($LRP_{20}$), 30\\% of the median biomass at the simulation estimate of maximum sustatainable yield ($LRP_{30}$), and 40\\% of the median biomass at the simulation estimate of maximum sustatainable yield ($LRP_{40}$). The USRs include, 40\\% of the median biomass using simulations with no fishing ($USR_{40}$) and 80\\% of the median biomass at the simulation estimate of maximum sustatainable yield ($USR_{80}$). The target removal reference was calculated as the exploitation rate which resulted in the maximum sustainable yield.")
```
