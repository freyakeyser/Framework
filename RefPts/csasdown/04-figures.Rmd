...
\newpage
```{r sab-ccf-m-rps-fig, echo=F, fig.cap="Relationship between fully-recruited natural mortality RPS in SFA 25A. The top panel is the time series of fully-recruited natural mortality (red dashed line) and RPS (text points, the years represent the year class of the recruits). The bottom panel is the cross-correlation, here the vertical black lines represent the strength of the relationship at each lag, the horizontal blue lines represent the 95\\% significance level"}
...
\newpage
```{r bbn-ccf-m-rps-fig, echo=F, fig.cap="Relationship between fully-recruited natural mortality RPS in SFA 26A. The top panel is the time series of natural mortality (red dashed line) and RPS (text points, the years represent the year class of the recruits). The bottom panel is the cross-correlation, here the vertical black lines represent the strength of the relationship at each lag, the horizontal blue lines represent the 95\\% significance level."}
...
```{r rp-bmi-fit, echo=F, fig.cap="Survey biomass index time series (black line) for each of the areas. The geometric mean for the most recent three-years of data is represented by the thick yellow line. The LRPs are represented by the red dotted line ($B_{MSY_{30}}$), the dashed red line ($B_{MSY_{34}}$), and the solid red line ($B_{0_{20}}$). The USRs are represented by the dashed blue line ($B_{MSY_{80}}$) and the solid blue line  ($B_{0_{40}}$)."}
...
\newpage
```{r sab-sim-ccf-fig, echo=F, fig.cap="Mean estimate of the cross-correlation of RPS and fully-recruited natural mortality from the MSY simulation time series in SFA 25A using the $$B_{msy}$$ scenario. The vertical black lines represent the strength of the relationship at each lag."}
...
\newpage
```{r sab-mod-rps-fig, echo=F, fig.cap="The historically modelled fully-recruited biomass time series (black line) relative to a suite of potential reference points in SFA 25A developed using the MSY simulations. The LRPs are represented by the red dotted line ($B_{MSY_{30}}$), the dashed red line ($B_{MSY_{34}}$), and the solid red line ($B_{0_{20}}$). The USRs are represented by the dashed blue line ($B_{MSY_{80}}$) and the solid blue line  ($B_{0_{40}}$). The solid yellow line is the estimated Biomass at MSY."}
...
\newpage
```{r sab-mod-reals-rps-fig, echo=F, fig.cap="MSY simulation time series projections of the fully-recruited biomass in SFA 25A when harvesting occurs at $RR_{tar}$.  A LRP ($B_{0_{20}}$; solid red line) and USR ($B_{0_{40}}$; solid blue line) are shown for reference. Each grey line represents one of teh 5000 realizations, and the orange line is the median of the time series."}
...
\newpage
```{r bbn-sim-ccf-fig, echo=F, fig.cap="Mean estimate of the cross-coreelation of RPS and fully-recruited natural mortality from the MSY simulation time series in SFA 26A using the $$B_{msy}$$ simulation run. The vertical black lines represent the strength of the relationship at each lag."}
...
\newpage
```{r bbn-mod-rps-fig, echo=F, fig.cap="The historically modelled fully-recruited biomass time series (black line) relative to a suite of potential reference points in SFA 26A developed using the MSY simulations. The LRPs are represented by the red dotted line ($B_{MSY_{30}}$), the dashed red line ($B_{MSY_{34}}$), and the solid red line ($B_{0_{20}}$). The USRs are represented by the dashed blue line ($B_{MSY_{80}}$) and the solid blue line  ($B_{0_{40}}$). The solid yellow line is the estimated biomass at MSY."}
...
\newpage
```{r bbn-mod-reals-rps-fig, echo=F, fig.cap="MSY Simulation time series of fully-recruited biomass projections in SFA 26A when harvesting occurs at $RR_{tar}$. A LRP ($B_{0_{20}}$; solid red line) and USR ($B_{0_{40}}$; solid blue line) are shown for reference.  Each grey line represents one of the 5000 realizations, and the orange line is the median."}
...
\newpage
```{r sab-real-c-hcr-fig, echo=F, fig.cap="HDR simulated raw removal time series in SFA 25A. Each grey line represents one of the 5000 realizations."}
...
\newpage
```{r bbn-real-c-hcr-fig, echo=F, fig.cap="HDR simulated raw removal time series in SFA 26A. Each grey line represents one of the 5000 realizations."}
