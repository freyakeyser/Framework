...
bbn.rps.sim.comp.plt <- ggplot(bbn.res.4.plts) + geom_point(aes(x=SSB/1000,y=rps.offset),alpha=alphs,shape=21,fill='black',stroke=NA) + 
                         geom_vline(xintercept = bbn.rps.bp/1000,color='red',linetype='solid') +
...
bbn.hcr.last.yrs <- bbn.hcr$Exp.res[bbn.hcr$Exp.res$year > (n_y-last.yrs),]
bbn.prop.yrs.bl.lrp.hcr <- round(100*length(which(bbn.hcr.last.yrs$B < bbn.lrp.b0))/n_sims/last.yrs,digits=2)
...
bbn.min.healthy.zone.catch.hcr <- bbn.usr.b0*0.025
bbn.max.healthy.zone.catch.hcr <- bbn.trp.bmsy*0.025
