...
prod.dat <- data.frame(Year = years[-length(years)],
                        ...
                        R = seam.mod$report$totR[-length(seam.mod$report$totR)], # 
                        #R = seam.mod$report$totR, # 
...
plot.url <- paste0(rp.loc,"Figures/BBn/R_75_FR_90/SEAM_",seam.select,"/")
# Make all things 2020 an NA
prod.dat[prod.dat$Year == 2020,-1] <- NA
...
p.rps.ssb <- ggplot(prod.dat,aes(y=RPS3,x=SSB)) + geom_text(aes(label = substr(Year,3,4)),size=3) + 
                                                  ...
                                                  scale_y_log10() + geom_vline(xintercept = 10000,size=0.5,color='firebrick2',linetype='dashed')  + 
...
ann.exploit <- ann.exploit %>% dplyr::filter(!year %in% 2020:2021)
...
r.strat <- data.frame(type = "bp",strat = 'dist', bp = 0.9e4,mn.mx = 0.1*signif(min(prod.dat$RPS3,na.rm=T),digits=2), sd.mx = 0.2,rec.age=3,
...
m.strat <- data.frame(type = "bp",strat = 'dist', bp = 1.0e4,mn.mx = 2*signif(max(prod.dat$m,na.rm=T),digits=2), sd.mx = 0.1, ar1=signif(p.m.pacf$data$Freq[1],digits=2),ar2=0) #
...
Res <- proj.mod(n_sim=500,n_y = 200,model=mod,plot_url = plot_url, run = 'no_model_error', res_url = res_url,save.results = T, exp.scenario=seq(0,0.24,0.01), mod.run = seam.select,
                #base.yrs = 2009:2022,
...
Exp.res <- readRDS("D:/Framework/SFA_25_26_2024/RefPts/Results/BBn/R_75_FR_90/SEAM_1994_2022_vary_m_m0_0.2_qR_0.45_20_knots_g_original/RPs/SEAM_g_sv__1_dist_1_0.05_r_bp_dist_9000__0.00041_0.23_m_bp_dist_10000__0.194_0.1.Rds")
...
T.lrp <- res.T.lrp %>% dplyr::group_by(scenario) %>% dplyr::summarise(T.lrp = min(years)) # Hmm...
T.trp <- res.T.trp %>% dplyr::group_by(scenario) %>% dplyr::summarise(T.trp = min(years)) # This tells me that TRP is really unachievable
...
hcr.strat <-data.frame(TRP = TRP.B.msy,TRP.exp = 0.13,
                       USR = USR.B0, USR.exp = 0.05, 
                       LRP = LRP.3.BMSY, LRP.exp = 0,exp.sd = 0.25)
#g.strat <- data.frame(type = "cor",strat = 'dist', bp = 9000,mn.mx = 1, sd.mx = 0.05,ar1=0,ar2=0) # The g correlation method seems to get g a bit too  high.
g.strat <- data.frame(type = "sv",set.value = 1,strat = 'dist', mn.mx = 1, sd.mx = 0.05)
#g.strat <- data.frame(type = "lm",strat = "Linear", bp = "Model",mn.mx = 1, sd.mx = 0.05)
r.strat <- data.frame(type = "bp",strat = 'dist', bp = 0.9e4,mn.mx = 0.1*signif(min(prod.dat$RPS3,na.rm=T),digits=2), sd.mx = 0.2,rec.age=3,
...
# An HCR model
hcr <- proj.mod(n_sim=5,n_y = 100,model=mod,plot_url = plot_url, run = 'model_error', res_url = res_url,save.results = T,mod.run = seam.select,
