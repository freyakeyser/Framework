...
seam.select <- "1994_2023_vary_m_m0_0.15_qR_0_1_10_knots"
...
g.strat <- data.frame(type = "bp",strat = 'dist', bp = 11000,mn.mx = 1, sd.mx = 0.05)
#g.strat <- data.frame(type = "lm",strat = "Linear", bp = "Model",mn.mx = 1, sd.mx = 0.05)
r.strat <- data.frame(type = "cor",strat = 'dist', bp = 11000,mn.mx = signif(min(prod.dat$RPS3,na.rm=T),digits=2), sd.mx = 0.2,rec.age=5,
...
m.strat <- data.frame(type = "cor",strat = 'dist', bp = 11000,mn.mx = signif(max(prod.dat$m,na.rm=T),digits=2), sd.mx = 0.1,ar1=signif(p.m.pacf$data$Freq[1],digits=2),ar2=-0) #
...
Res <- proj.mod(n_sim=50,n_y = 100,model=mod,plot_url = plot_url, run = 'model_error', res_url = res_url,save.results = T, exp.scenario=seq(0,0.3,0.02),
