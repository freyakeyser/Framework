...
tlm.select <- "1994_2022_qR_0_5"
tlm.mod <- readRDS(paste0(repo_loc,"BBn_model/Results/Models/BBn/BBn_TLM_model_output_",tlm.select,".Rds"))
...
seam.select <- "1994_2022_vary_m_m0_1_R0_150_10_knots"
seam.mod <- readRDS(paste0(repo_loc,"BBn_model/Results/Models/BBn/BBn_SEAM_model_output_",seam.select,".Rds"))
...
plot.url <- paste0("D:/Github/BBn_model/Results/Figures/BBn/Ref_points/",seam.select,"/")
...
m.comp <- ggplot(nat.mat.long,aes(x= Year,y=m,group=cat,color=cat)) + geom_point(size=3) + geom_line(size=1.5) + 
...
# OK, so we see recruits are probably 5-6 years old based on that von B, so we could use either RPS5 or RPS6 for the density dependence 
...
p.rps.ts <- ggplot(prod.dat, aes(x=Year,y=RPS5))  + geom_point() + 
...
# Density dependence 
p.rps.ssb <- ggplot(prod.dat,aes(y=RPS5,x=SSB)) + geom_text(aes(label = substr(Year,3,4)),size=3) + 
                                                  xlab("Spawning Stock Biomass (tonnes)") + ylab("Recruits per Spawner (kg\u00B9\u22C5kg\u207B\u00B9)") +
                                                  geom_smooth(method = 'lm') +
                                                  scale_y_log10() + geom_vline(xintercept = 12000,size=0.5,color='firebrick2',linetype='dashed')  + 
...
#ggplot(prod.dat,aes(y=RPS5,x=SSB)) + geom_text(aes(label = substr(Year,3,4)))  + geom_smooth(method = 'lm') + scale_y_log10()  + scale_x_continuous(breaks = c(seq(1000,20000,by=500)))
#ggplot(prod.dat,aes(y=RPS5,x=SSB)) + geom_text(aes(label = substr(Year,3,4)))  + geom_smooth(method = 'lm')  + scale_x_continuous(breaks = c(seq(1000,20000,by=500)))
...
#ggplot(prod.dat,aes(y=RPS5,x=SSB)) + geom_text(aes(label = substr(Year,3,4)))  + geom_smooth(method = 'gam') + scale_y_log10() + scale_x_continuous(breaks = c(seq(1000,20000,by=500)))
#ggplot(prod.dat,aes(y=RPS5,x=SSB)) + geom_text(aes(label = substr(Year,3,4)))  + geom_smooth(method = 'gam')  + scale_x_continuous(breaks = c(seq(1000,20000,by=500)))
...
p.rec.ssb <- ggplot(prod.dat,aes(y=R5,x=SSB)) + geom_text(aes(label = substr(Year,3,4)),size=3)  +
                                                ...
                                                geom_vline(xintercept = 13500,size=0.5,color='firebrick2',linetype='dashed')  + 
...
#ggplot(prod.dat,aes(y=R5,x=SSB)) + geom_text(aes(label = substr(Year,3,4)))  + geom_smooth(method = 'gam') + scale_x_continuous(breaks = c(seq(1000,20000,by=500)))
...
p.rps.cf <- ggplot(prod.dat ,aes(y=RPS5,x=CF)) + geom_text(aes(label = substr(Year,3,4))) + 
...
rps.mod <- lm(log(RPS5) ~ I(Year-1993),data=prod.dat)
...
# Natural morality correlation? Yep, autocorrelation very similar to recruitment
...
# What about other lags... Interesting that a 5 year lag shows up between mortality and RPS5.  
...
prod.dat$RPS5_offset <- c(rep(NA,5),na.omit(prod.dat$RPS5[]))
ggplot(prod.dat,aes(x=Year,y=RPS5)) +geom_point() + geom_line(size=2) + geom_point(aes(y = m)) + geom_line(aes(y = m),color='blue') #+ geom_smooth(method = 'gam')
ggplot(prod.dat,aes(x=m,y=RPS5)) + geom_text(aes(label = substr(Year,3,4))) + geom_smooth(method = 'gam')
...
# relationship there.
p.rpsoff.m <- ggplot(prod.dat,aes(x=Year,y=RPS5_offset)) + geom_text(aes(label = substr(Year-5,3,4)))  + 
                                                           xlab("") +ylab("Recruits per Spawner (kg\u00B9\u22C5kg\u207B\u00B9) & Natural mortality (Inst)") +
                                                           geom_line(aes(y = m),color='firebrick2',size=1,linetype='dashed')
p.rps.m.ccf <- ggCcf(prod.dat$m,prod.dat$RPS5_offset) + ggtitle("")
...
# ccf(prod.dat$RPS5,prod.dat$g,na.action= na.omit)
# ccf(prod.dat$RPS5,prod.dat$gR,na.action= na.omit)
...
ann.exploit <- data.frame(year = years[-length(years)],B = exp(pred.proc.seam$log_tot_frame$log_totB)[-length(years)], Catch = colSums(catchy),
...
phase.plt <- ggplot(ann.exploit,aes(x=B,y=exploit)) + geom_text(aes(label=year)) +  xlim(c(0,15000)) + 
                                    ...
                                    xlab("Biomass (Tonnes)") + ylab("Exploitation Rate (Proportional)") 
save_plot(paste0(plot.url,"Phase_plot.png"),kobe.plt,base_height =12,base_width = 10)                
...
g.strat <- data.frame(type = "bp",strat = 'dist', bp = 10000,mn.mx = 1, sd.mx = 0.05)
#g.strat <- data.frame(type = "lm",strat = "Linear", bp = "Model",mn.mx = 1, sd.mx = 0.05)
r.strat <- data.frame(type = "cor",strat = 'dist', bp = 12000,mn.mx = signif(min(prod.dat$RPS5,na.rm=T),digits=2), sd.mx = 0.2,rec.age=5,
                      ar1=signif(p.rps.pacf$data$Freq[1],digits=2),ar2=signif(p.rps.pacf$data$Freq[2],digits=2),rps.m.cor = 0.53,rps.m.lag = -5) # Note lag is -5, high RPS comes 5 years before high m due to RPS offset.
#r.strat <- data.frame(type = "bp",strat = 'dist', bp = 13500,mn.mx = signif(min(prod.dat$RPS5,na.rm=T),digits=2), sd.mx = 0.2,rec.age=5,
...
m.strat <- data.frame(type = "cor",strat = 'dist', bp = 10000,mn.mx = signif(max(prod.dat$m,na.rm=T),digits=2), sd.mx = 0.1,ar1=signif(p.m.pacf$data$Freq[1],digits=2),ar2=-0) #
...
Res <- proj.mod(n_sim=500,n_y = 100,model=mod,plot_url = plot_url, run = 'model_error', res_url = res_url,save.results = T, exp.scenario=seq(0,0.3,0.02),
...
hcr.strat <-data.frame(TRP = 5000,TRP.exp = 0.127,
                       USR = 0, USR.exp = 0.127, 
                       LRP = 0, LRP.exp = 0.127,exp.sd = 0.25)
#g.strat <- data.frame(type = "cor",strat = 'dist', bp = 9000,mn.mx = 1, sd.mx = 0.05,ar1=0,ar2=0) # The g correlation method seems to get g a bit too  high.
g.strat <- data.frame(type = "bp",strat = 'dist', bp = 10000,mn.mx = 1, sd.mx = 0.05)
#g.strat <- data.frame(type = "lm",strat = "Linear", bp = "Model",mn.mx = 1, sd.mx = 0.05)
r.strat <- data.frame(type = "cor",strat = 'dist', bp = 12000,mn.mx = signif(min(prod.dat$RPS5,na.rm=T),digits=2), sd.mx = 0.2,rec.age=5,
